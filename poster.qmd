...
# 3) Create country mapping for milestone events
# Using multiple naming variations to match different dataset conventions
country_groups <- list(
  "1989" = c("Germany", "Poland", "Hungary", "Czechoslovakia", "Czech Republic", "Czechia", "Slovakia", "Bulgaria", "Romania", "East Germany", "West Germany"),
  "1994" = c("Rwanda", "Burundi", "Democratic Republic of Congo", "Congo, Dem. Rep.", "Uganda", "Tanzania", "Congo", "Central African Republic"),
  "2001" = c(), # Global event - no specific highlighting
  "2004" = c("Indonesia", "Thailand", "Sri Lanka", "India", "Maldives", "Myanmar", "Bangladesh", "Malaysia", "Somalia", "Kenya", "Seychelles"),
  "2008" = c(), # Global event - no specific highlighting
  "2010" = c("Tunisia", "Egypt", "Egypt, Arab Rep.", "Libya", "Syria", "Syrian Arab Republic", "Yemen", "Yemen, Rep.", "Bahrain", "Algeria", "Jordan", "Morocco", "Lebanon", "Sudan"),
  "2014" = c("Guinea", "Liberia", "Sierra Leone", "Mali", "Nigeria", "Senegal", "Democratic Republic of Congo", "Congo, Dem. Rep."),
  "2015" = c(), # Global event - no specific highlighting
  "2016" = c(), # Global event - no specific highlighting
  "2020" = c(), # Global event - no specific highlighting
  "2022" = c() # Global event - no specific highlighting
)
...
# 7) Enhanced JavaScript with unified animation control and country highlighting
widget <- widget %>%
  htmlwidgets::onRender("
    function(el, x) {
      ...
      var milestoneYears = [1989, 1994, 2001, 2004, 2008, 2010, 2014, 2015, 2016, 2020, 2022];
      var milestoneTexts = {
        1989: 'Fall of Berlin Wall:<br>End of Cold War era, beginning<br>of major political transitions<br><span style=\"color:#d62728;font-weight:bold;\">→ Highlighted: Eastern European countries</span>',
        1994: 'Rwanda Genocide:<br>Humanitarian crisis highlighting<br>need for conflict prevention<br><span style=\"color:#d62728;font-weight:bold;\">→ Highlighted: Central/East African countries</span>',
        2001: 'Millennium Development Goals:<br>Global commitment to<br>reducing poverty and inequality<br><span style=\"color:#666;font-style:italic;\">Global impact - all countries relevant</span>',
        2004: 'Indian Ocean Tsunami:<br>Natural disaster affecting<br>development in affected regions<br><span style=\"color:#d62728;font-weight:bold;\">→ Highlighted: Indian Ocean rim countries</span>',
        2008: 'Global Financial Crisis:<br>Economic downturn affecting<br>development worldwide<br><span style=\"color:#666;font-style:italic;\">Global impact - all countries relevant</span>',
        2010: 'Arab Spring Begins:<br>Political upheaval affecting<br>Middle East and North Africa<br><span style=\"color:#d62728;font-weight:bold;\">→ Highlighted: MENA countries</span>',
        2014: 'Ebola Outbreak:<br>Health crisis in West Africa<br>affecting regional development<br><span style=\"color:#d62728;font-weight:bold;\">→ Highlighted: West African countries</span>',
        2015: 'Sustainable Development Goals:<br>New framework for global<br>development agenda<br><span style=\"color:#666;font-style:italic;\">Global impact - all countries relevant</span>',
        2016: 'Paris Climate Agreement:<br>Global commitment to<br>climate action and sustainability<br><span style=\"color:#666;font-style:italic;\">Global impact - all countries relevant</span>',
        2020: 'COVID-19 Pandemic:<br>Global health crisis impacting<br>human development progress<br><span style=\"color:#666;font-style:italic;\">Global impact - all countries relevant</span>',
        2022: 'Russia-Ukraine War:<br>Major conflict affecting<br>global economy and development<br><span style=\"color:#666;font-style:italic;\">Global impact - all countries relevant</span>'
      };
      ...
      function highlightCountries(year, countries) {
        if (!gd.data || !gd.data[0] || !countries || countries.length === 0) {
          return;
        }
        ...
        var frameData = null;
        if (gd._fullLayout && gd._fullLayout.sliders && gd._fullLayout.sliders[0] && gd._fullLayout.sliders[0].steps) {
          var steps = gd._fullLayout.sliders[0].steps;
          var currentStep = steps.find(function(step) {
            return step.label === year.toString();
          });
          if (currentStep && currentStep.args && currentStep.args[0]) {
            frameData = currentStep.args[0];
          }
        }
        
        // If we can't get frame data, work with current visible data
        var currentData = frameData || gd.data[0];
        var textData = currentData.text || gd.data[0].text;
        
        if (!textData) return;
        
        // Create opacity and line width arrays
        var opacities = [];
        var lineWidths = [];
        var lineColors = [];
        ...
        for (var i = 0; i < textData.length; i++) {
          ...
          if (match) {
            ...
            // Check for exact matches and partial matches (for variations like 'Korea, Rep.' vs 'South Korea')
            isHighlighted = countries.some(function(country) {
              return countryName === country || 
                     ...
                     (country === 'Russia' && countryName.includes('Russian')) ||
          ...
          if (isHighlighted) {
            opacities.push(1.0);
            lineWidths.push(4);
            lineColors.push('#d62728'); // Red highlight
            highlightedCount++;
          } else {
            opacities.push(0.15); // More faded non-highlighted countries
            lineWidths.push(1);
        ...
        console.log('Year:', year, 'Expected countries:', countries, 'Highlighted count:', highlightedCount);
      ...
      function resetHighlighting() {
        if (!gd.data || !gd.data[0] || !originalMarkerData) {
          return;
        }
        
        Plotly.restyle(gd, {
          'marker.opacity': originalMarkerData.opacity,
          'marker.line.width': originalMarkerData.line.width,
          'marker.line.color': originalMarkerData.line.color
        });
      }
      ...
      if (picker && picker.options.length === 0 && years && years.length) {
        ...
        picker.onchange = function() {
          ...
          resetHighlighting(); // Reset any highlighting
          if (gd) {
